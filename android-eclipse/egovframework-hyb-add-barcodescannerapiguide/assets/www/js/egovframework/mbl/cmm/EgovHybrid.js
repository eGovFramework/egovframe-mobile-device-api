/************************************************************************
  ...
  2016.06.21   신용호         1.1              window.plugins.EgovInterface.request로 iOS, Android 공통 function 추가 
...
function backKeyDown(e) {
    ...
    } else if ($.mobile.activePage.is('#BarcodescannerInfo') || $.mobile.activePage.is('#BarcodescannerList')) {
	    location.href = 'Barcodescanner.html';
	} else {
 ...
 * @type
*/
EgovInterface.prototype.get = function(url, accept_type, param, successCallback, failCallback) {
	
	if(param == null)
		param = {};
	var defaultFailCallback = function(e){console.log('DeviceAPIGuide EgovInterface.get request Fail');jAlert(e);};
	
	if (failCallback == undefined) failCallback = defaultFailCallback;
 ...
 * @type
*/
EgovInterface.prototype.post = function(url, accept_type, param, successCallback, failCallback) {
	
	if(param == null)
		param = {};
	var defaultFailCallback = function(e){console.log('DeviceAPIGuide EgovInterface.post request Fail');jAlert(e);};
...
EgovInterface.prototype.request = function(url, param, successCallback, failCallback) {

	var accept_type = "json";
	if(param == null)
		param = {};
	var defaultFailCallback = function(e){console.log('DeviceAPIGuide EgovInterface.post request Fail');jAlert(e);};
	var defaultSuccessCallback = function(stringData){
    ...
    return cordova.exec(    
    		defaultSuccessCallback,    						//Success callback from the plugin
    		failCallback,     //Error callback from the plugin
    		'EgovInterfacePlugin',  					//Tell PhoneGap to run "EgovInterfacePlugin" Plugin
    		'POST',              						//Tell plugin, which action we want to perform
    		[url,accept_type,param]);        			//Passing list of args to the plugin
